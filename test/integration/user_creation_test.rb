...
class UserCreationTest < ActionDispatch::IntegrationTest
  ...
  # email from the oauth details. No email or password provided
  # no email sent for confirmation. 
  def test_user_create_google_no_email_no_password_success
    OmniAuth.config.add_mock(:google, :uid => "123454321", 
                            :info => { :name => "new_tester-google", :email=> "newtester-google@osm.org"},
                            :extra => {
                               :id_info => { "openid_id" => "http://localhost:1123/new.tester" }
                             })

    new_email = "newtester-google@osm.org"
    display_name = "not real name"
    ...
    assert_difference("User.count", 1) do
      assert_difference("ActionMailer::Base.deliveries.size", 0) do
        perform_enqueued_jobs do
          ...
          post "/user/new",
          :params => { :user => { :display_name => display_name, :auth_provider => "google", :pass_crypt => "", :pass_crypt_confirmation => "" } }
          assert_response :redirect
          ...
          assert_redirected_to "/user/terms"
          post "/user/save",
               :params => { :user => { :display_name => display_name, :auth_provider => "google", :auth_uid => "123454321" }, :read_ct => 1, :read_tou => 1 }
          assert_response :redirect
          follow_redirect!
        end
      end
    ...
    assert User.last.email, new_email
    assert User.last.display_name, display_name

    ActionMailer::Base.deliveries.clear
  end

  # no mail, and welcome, even with different email passed in to form which shouldnt be read
  def test_user_create_google_success
    actual_email = "newtester-google@osm.org"
    OmniAuth.config.add_mock(:google, :uid => "123454321", 
                              :info => { :name => "new_tester-google", :email=> actual_email },
                              :extra => {
                               :id_info => { "openid_id" => "http://localhost:1123/new.tester" }
                             })

    new_email = "wrongemail-google@osm.org"
    ...
    assert User.last.email, actual_email
    assert User.last.display_name, display_name
  ...
  def test_user_create_google_redirect
    OmniAuth.config.add_mock(:google, :uid => "123454321", 
                                :info => { :name => "new_tester-google", :email=> "redirect_tester_google@osm.org"},
                                :extra => {
                               :id_info => { "openid_id" => "http://localhost:1123/new.tester" }
                             })
    ...
    def test_google_no_tou_failure
      OmniAuth.config.add_mock(:google, :uid => "123454321", 
        :info => { :name => "new_tester-google", :email=> "newtester-google@osm.org"},
        :extra => {
           :id_info => { "openid_id" => "http://localhost:1123/new.tester" }
  ...
  def test_user_create_facebook_disabled
  ...
  def test_user_create_windowslive_success_disabled
  ...
  def test_user_create_github_disabled
  ...
  def test_user_create_wikipedia_disabled
